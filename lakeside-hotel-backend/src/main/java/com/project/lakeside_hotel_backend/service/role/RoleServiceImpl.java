...
import com.project.lakeside_hotel_backend.repository.RoleRepository;
import com.project.lakeside_hotel_backend.repository.UserRepository;
...
public class RoleServiceImpl implements IRoleService {
    ...
    public Role createRole(Role theRole) {
        ...
        if (roleRepository.existsByName(roleName)) {
            throw new RoleAlreadyExistsException(theRole.getName() + " already exists");
        }
    ...
    public User removeUserFromRole(Long userId, Long roleId) {
        ...
        if (role.isPresent() && role.get().getUsers().contains(user.get())) {
            role.get().removeUserFormRoom(user.get());
    ...
    public Role removeAllUsersFromRole(Long roleId) {
        Optional<Role> role = roleRepository.findById(roleId);
        role.get().removeAllUsersFromRoom();
